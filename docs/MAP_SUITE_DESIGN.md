...
版本 v1.2 · 2026-07-18（v1.0：初版；v1.1 依作者方向反馈重定位：AI-first 编辑与种群种子、按需物化、性能预算——新增 §1.6-1.8、§2.1⓪，重构 §4，调整 §6/§7；v1.2 新增 §1.9 数据层依赖模型——分层 DAG、双平面四类节点、时序展开、失效膜）
...
### 1.9 数据层依赖模型（v1.2 新增，流程细化的基准）

一切流程、缓存失效、AI 介入点、惰性物化的裁决依据都是这张图。两条支柱已有实现：**场平面**（graphdag.py：CellField(frequency) 算子 DAG、内容寻址缓存、边频率硬校验）与**实体平面**（worldstore.py：stable_id 实体、gen/user 双层、锁、apply_generation 合并契约）。依赖模型 = 在这两个平面上定义四类节点、三类边、五个层、若干失效膜。

**① 四类节点。**

- **F 场节点**：确定性算子输出，`hash(op, params, 上游hash)` 定址，重算无副作用、无协商。
- **E 实体集节点**：一个生成 phase 的 `apply_generation` 批（stable_id(kind, gen_key)）。重算永远走合并契约——user 层与锁不可被生成器覆写。
- **C 约束节点（源节点）**：种群包、suitability filter、区域掩膜、place_constraint、锁、风格包、reseed 指令。这是人类/AI 意图进入依赖图的**唯一入口**（§1.6），存 user 层，永不被重算失效；但参与下游节点的 hash——改约束即精确标脏其失效锥。
- **D 派生场节点**：实体→场的栅格化回写（accessibility、领土 id、路网 cell 层——civ.py v1 的 cell_polity_capital/cell_road 即此类）。上游实体集变则重算。

推论：**AI 只写 C 节点**，F/E/D 全部是 (seed, C, 上游) 的确定性函数——§1.5 确定性纪律与 §2.1⓪ "管线外产参数、管线内零 AI" 在依赖图上的形式化表述。

**② 分层 S0–S4（对作者链条的三处修正）。**

```mermaid
graph TD
  subgraph S0["S0 物理层（场平面，已实现）"]
    TEC[构造模拟] --> FP["地形⇄气候⇄水文⇄侵蚀<br/>耦合固定点（physics 节点）"]
    FP --> FLD["8 正典场 + 河/湖实体"]
  end
  subgraph S1["S1 生物圈层（AI 首次介入）"]
    FLD --> BIO["cell_biome（确定性，零 AI）"]
    BIO --> ECO["生态区实体<br/>（连通域分割 + AI 命名/风味）"]
    SPK[/"C：种群包 filter（AI 编译）"/] --> SUIT["种群适宜度场（F）"]
    FLD --> SUIT
    MSK[/"C：区域掩膜（AI 圈选/作者）"/] --> SUIT
    ECO -.可被掩膜引用.-> MSK
  end
  subgraph S2["S2 文明宏观层（era 展开）"]
    SUIT --> SEED["聚落种子批 era_k（E）"]
    SEED --> ROAD["道路连接 era_k"]
    ROAD --> ACC["accessibility（D）"]
    ACC -->|"era_{k+1}"| SEED
    SEED --> HIER["层级/人口"]
    HIER --> CUL["文化 hearth 扩散"] --> POL["势力/边境（cost-field）"]
    POL --> TRUNK["长途路网定级（gravity）"]
    HIER --> TRUNK
  end
  subgraph S34["S3 中观（半惰性）/ S4 局部（全惰性）"]
    POL --> PROV["省份/宗教/外交编年"]
    SUIT --> SECT["宗门等组织实体"]
    HIER --> CITY["城市详情"]
    FLD --> DEM["米级 DEM patch"]
    CITY --> L3["L2/L3 布局"]
    DEM --> L3
    L3 --> PROSE["文案/prose"]
  end
...
- **S0 物理层**（场平面，M0–M6 已有）。修正①：地形→水文→气候**不是链而是耦合环**——降水依赖地形雨影，径流依赖降水，侵蚀反塑地形。kernel 已用固定点迭代内部消解（physics 节点 `fixed_point_iterations`）。对外契约：S0 是**一个原子段**，只暴露收敛后的正典场；"基础水文"（纯高程流向拓扑）确可先行，但通量必须留在环内。
- **S1 生物圈层**。修正②：生态圈拆成**确定性半 + 世界观半**。cell_biome（温度×降水分类）是确定性 F 节点，留在场平面；生态区实体 = cell_biome × 地理连通域分割 + AI 命名/风味化。雪豹例的形式化：种群分布场 = suitability filter（高原∧雪山∧寒冷——全球任何满足处都命中，F 节点）**∩ 区域掩膜**（"欧亚大陆，喜马拉雅—中亚"——可引用、可锁、可重画的一等 C 节点；来源可以是 NL→geoquery 圈选或生态区 id 列表）。局部修改能力即掩膜编辑，不碰 filter 本身。
- **S2 文明宏观层**。修正③：**路网出现两次**。局部连接性在 era 循环内与聚落共演化（Emilien 正反馈，§2.1②）；长途干线**定级**在势力之后（gravity 要读全量人口，干线偏好受边境影响）。作者链条"长途路网在势力后"对定级成立，对连接性不成立。
- **S3 中观 / S4 局部**：城市详情、宗门、省份、宗教、外交编年（半惰性）；米级 DEM patch → L2/L3 布局 → 文案（全惰性，§1.7）。宗门 = 组织实体，与聚落**平行**锚定：自身 suitability（偏远×险峻/灵秀，可用种群包同款 filter DSL 表达）+ 可选聚落耦合（招收半径）。

**③ 环→DAG 的关键手法：时序展开。** settlement⇄road⇄accessibility、境⇄路都是真实反馈环。按纪元展开后，环内的反馈边全部改指 era_{k+1}，图恢复无环；era_k 的 hash 含 era_{k-1} 的全部输出，改早期纪元的失效锥自动覆盖后续纪元。`reseed(scope)` 的作用域语义即"从纪元 k、区域 R 起的下游锥"。

**④ 三类边与失效膜。**

| 边 | 例 | 失效行为 |
|---|---|---|
| 纯函数边（F→F、F→D、E→D） | 场算子、栅格化回写 | 内容寻址，脏即重算，无协商 |
| 采样边（F+C→E） | 适宜度场→种子批 | 上游变**不**自动重算：先出失效锥预告+diff 预览（§1.2/§1.8）；应用时走 apply_generation 合并 + §1.3 重锚定——未锁 gen 实体重推导，锁定/正典实体重锚或报冲突，绝不静默覆写 |
| 纪元边（era_k→era_{k+1}） | accessibility 回写 | 只向前；scoped reseed 的天然边界 |

失效膜（invalidation 变性的位置）：S0 内部自由重算；**S0→S1/S2 膜**上失效降级为"提案"（预览→批准）；**S3/S4 一旦正典**（被手稿引用）不再自动失效，上游矛盾进冲突审计清单（§5.10 audit 规则的输入端）。

**⑤ 工程落地：双层图 + provenance 签名。**

- 下层：场 DAG（graphdag.py 现状）承载 S0 与一切 F/D 节点——suitability、accessibility 回写注册为新算子即可。
